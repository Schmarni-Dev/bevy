...
fn oit_draw(position: vec4f, color: vec4f, premultiplied: bool) {
    ...
    let depth_alpha = pack_24bit_depth_8bit_alpha(position.z, color.a, premultiplied);
    oit_layers[layer_index] = vec2(rgb9e5_color, depth_alpha);
}
#endif // OIT_ENABLED

fn pack_24bit_depth_8bit_alpha(depth: f32, alpha: f32, premultiplied: bool) -> u32 {
    let depth_bits = u32(saturate(depth) * f32(0xFFFFFFu) + 0.5);
    let alpha_bits = u32(saturate(alpha) * f32(0xFFu) + 0.5);
    return (depth_bits & 0xFFFFFEu) | ((alpha_bits & 0xFFu) << 24u) | (u32(premultiplied));
}

fn unpack_24bit_depth_8bit_alpha(packed: u32) -> vec2<f32> {
    let depth_bits = packed & 0xFFFFFEu;
    let alpha_bits = (packed >> 24u) & 0xFFu;
    return vec2(f32(depth_bits) / f32(0xFFFFFEu), f32(alpha_bits) / f32(0xFFu));
}

fn unpack_premultiplied(packed:u32) -> bool {
    return (packed & 1u) == 1;
}
